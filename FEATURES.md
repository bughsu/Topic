...
這是一個基於 Qt6 和 FFmpeg 的專業多路監控錄影系統，整合了 OpenCV 實現智慧移動偵測功能，並支援遠端 Web 監看。

## 主要功能

### 1. 遠端 Web 監看 🌐 (新功能)
- 內嵌輕量級 HTTP Server（基於 Qt Network 的 QTcpServer）
- 即時將視訊畫面轉碼為 MJPEG 格式
- 透過 HTTP 串流輸出，支援標準瀏覽器播放
- 自動偵測並顯示本機 IP 地址
- 提供簡潔美觀的網頁介面
- 支援多個客戶端同時連接觀看
- 約 30 FPS 的串流幀率
...
#### WebServer（Web 伺服器）
- 基於 QTcpServer 實作的輕量級 HTTP Server
- 處理 HTTP 請求並路由到對應處理函數
- MJPEG 串流編碼與傳輸
- 多客戶端連接管理
- 自動偵測本機 IP 地址
...
1. 確保至少有一個串流正在播放
2. 點擊「啟用遠端監看」按鈕
3. 系統會顯示訪問網址（例如：http://192.168.1.100:8080）
4. 在手機或平板的瀏覽器中輸入該網址
5. 即可即時觀看監控畫面
6. **注意**：手機/平板必須與電腦在同一個區域網路中

**提示**：
- 預設 port 為 8080，如被佔用會自動選擇其他 port
- 當有多個串流時，會顯示當前聚焦的畫面
- 如未聚焦任何畫面，會顯示第一個串流
- 支援多個客戶端同時連接觀看
...
客戶端連接
    ↓
QTcpServer 監聽 (Port 8080)
    ↓
HTTP 請求解析
    ↓
路由處理
    ├─ / (首頁) → HTML 頁面
    └─ /stream.mjpeg → MJPEG 串流
        ↓
    影格擷取 (每 500ms)
        ↓
    QImage → JPEG 編碼 (品質 85)
        ↓
    MJPEG 封裝 (multipart/x-mixed-replace)
        ↓
    HTTP 傳輸 (~30 FPS)
```

**關鍵技術**：
- HTTP Server: QTcpServer (Qt Network)
- 影像編碼: QImage::save (JPEG, 85% 品質)
- 串流格式: multipart/x-mixed-replace with boundary
- 幀率控制: QTimer (33ms interval ≈ 30 FPS)
...
- [ ] 雲端儲存整合
- [ ] 多使用者權限管理
- [x] ~~行動裝置 App 支援~~ → 已實現：遠端 Web 監看（無需安裝 App）
- [ ] HTTPS 加密連線支援
- [ ] 多串流畫面同時輸出到 Web
- [ ] 使用者認證和訪問控制
